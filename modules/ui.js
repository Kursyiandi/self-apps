...
export function updateTanggalHeader() {
    ...
    el.teksTanggalHeader.innerText = dateObj.toLocaleDateString('id-ID', { 
        weekday: 'long', 
        day: 'numeric', 
        month: 'long', 
        year: 'numeric' 
...
export function renderLayarUtama() {
    ...
    if (state.modeAktif === "catatan") {
        ...
        state.dataCatatanPribadi.forEach(data => {
            const isi = data.isi || "";
            const tampilkan = kataKunci !== "" ? isi.toLowerCase().includes(kataKunci) : data.tanggal_db === tglPilih;
        ...
        if (jumlahTampil === 0) {
            el.daftarItem.innerHTML = kataKunci !== "" 
                ? `<div class='pesan-kosong'>Pencarian tidak ditemukan.</div>` 
                : `<div class='pesan-kosong'>Tidak ada catatan.</div>`;
        } else {
            el.daftarItem.innerHTML = htmlCatatan;
        }
        ...
        state.dataPengeluaranPribadi.forEach(data => {
            if (data.tanggal_db && data.tanggal_db.startsWith(blnPilih)) {
                totalBulan += data.harga;
            }
        ...
        el.daftarItem.innerHTML = jumlahTampil === 0 
            ? `<div class='pesan-kosong'>Tidak ada pengeluaran hari ini.</div>` 
            : htmlHariIni;
        ...
        state.dataAkunPribadi.forEach(data => {
            ...
            const teksCari = p.toLowerCase() + " " + u.toLowerCase();
            const tampilkan = kataKunci !== "" ? teksCari.includes(kataKunci) : true;
            
            if (tampilkan) {
                jumlahTampil++;
                const teksPlatform = bersihkanTeks(p);
        ...
        if (jumlahTampil === 0) {
            el.daftarItem.innerHTML = kataKunci !== "" 
                ? `<div class='pesan-kosong'>Pencarian tidak ditemukan.</div>` 
                : `<div class='pesan-kosong'>Belum ada akun yang tersimpan.</div>`;
        } else {
            el.daftarItem.innerHTML = htmlAkun;
        }
    }
}
