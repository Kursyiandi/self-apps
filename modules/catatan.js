...
export async function simpanCatatan(teks, tgl) {
    if (!teks) { showToast("Catatan tidak boleh kosong!", "error"); return false; }
    if (!navigator.onLine) { showToast("Tidak ada koneksi internet!", "error"); return false; }

    const { data, error } = await supabase.from('catatan_pribadi').insert([{
        user_id: state.currentUser.id, isi: teks, tanggal_db: tgl, jam_input: formatWaktuDetail()
    }]).select();
    ...
    if (error) { 
        ...
        state.dataCatatanPribadi.unshift(data[0]); // Suntik data ke memori lokal
...
export async function hapusCatatan(idItem) {
    if (!navigator.onLine) { showToast("Tidak ada koneksi internet!", "error"); return false; }
    const { error } = await supabase.from('catatan_pribadi').delete().eq('id', idItem);
    if (!error) { 
...
export async function updateCatatan(idItem, teksBaru) {
    ...
    const { error } = await supabase.from('catatan_pribadi').update({ isi: teksBaru, jam_input: jamBaru }).eq('id', idItem);
    ...
    showToast("Gagal menyimpan!", "error"); return false;
}